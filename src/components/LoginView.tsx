...
import { User, Key, LogIn, Settings, AlertCircle, KeyRound, Users, GraduationCap, BookOpen, UserCheck, FileUp, HelpCircle, CheckCircle2, Download, Sparkles, Building2, Trophy, Crown, Medal, Award, Flame, ChevronDown, ChevronUp, FileSpreadsheet, RefreshCw, FileJson, Code2 } from 'lucide-react';
    ...
    <div className="flex-1 min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 p-3 sm:p-6 overflow-y-auto custom-scrollbar flex items-center justify-center">
      <div className="w-full max-w-6xl my-auto flex flex-col lg:flex-row gap-6 items-stretch justify-center">
        ...
        >
              ...
              {activeMode === 'student' ? (
                ...
                <>
                  ...
                  <div className="pt-2 flex flex-col gap-2.5">
                    ...
                    <label className="bg-amber-50/90 hover:bg-amber-100/90 active:bg-amber-200 text-amber-900 border border-amber-300 rounded-xl py-3 px-4 text-xs sm:text-sm font-bold flex items-center justify-center gap-2 cursor-pointer transition active:scale-[0.98] shadow-2xs">
                      <Code2 className="w-4.5 h-4.5 text-amber-700 shrink-0" />
                ...
                <>
                  <div className="flex items-center justify-between pb-1 border-b border-slate-100">
                    <span className="text-xs font-extrabold text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
                      <Settings className="w-4 h-4 text-indigo-600" /> Panel Pengelola Ujian
                    </span>
                    <button
                      type="button"
                      onClick={() => {
                  ...
                  {/* Quick Setting Ujian dengan File Paket JSON (Tampil hanya di Panel Pengelola) */}
          ...
          <div className="p-3.5 bg-slate-50 border-t border-slate-100 text-center text-[11px] text-gray-500 flex items-center justify-between gap-2 px-5 flex-wrap rounded-b-3xl">
            ...
            {/* Discrete Teacher Access Toggle Button */}
            {activeMode === 'student' ? (
              <button
                type="button"
                onClick={() => {
                ...
                className="text-[10px] font-bold text-slate-400 hover:text-slate-700 transition-colors flex items-center gap-1 cursor-pointer py-1 px-2 rounded-lg hover:bg-slate-200/60"
                title="Akses Panel Pengelola Ujian (Khusus Guru / Admin)"
              >
                <Settings className="w-3 h-3 text-slate-500" />
                <span>[ Login Guru / Pengelola ]</span>
              </button>
            ) : (
              <button
                type="button"
                ...
                className="text-[10px] font-bold text-blue-600 hover:text-blue-800 transition-colors flex items-center gap-1 cursor-pointer py-1 px-2 rounded-lg hover:bg-blue-50"
              >
                <Users className="w-3 h-3" />
                <span>[ Form Login Siswa ]</span>
              </button>
            )}
          </div>
        </div>
